...
| 3 | 🎙️ **人云议云** | 会议纪要 | 录音 → 语音转文字 → AI 自动生成会议纪要 |
| 4 | ✍️ **字斟句酌** | 报告润色 | 原文与润色结果左右对比，口语→商务/学术 |
| 5 | 🫵 **向上管理** | 汇报翻译 | 把吐槽翻译成老板爱看的语言（目标导向/风险预警/创新驱动） |
| 6 | ⚗️ **需求炼金** | 需求文档 | 零散笔记 → 结构化需求（规格列表 或 用户故事） |
...
```
┌──────────────────────────────────────────────────────────┐
│  🐂 牛马助手 v1.0            [设置] [登录]  🌐 中/EN     │
├──────────────────────────────────────────────────────────┤
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐   │
│  │🥱太长不看│ │💌已读乱回│ │🎙️人云议云│ │✍️字斟句酌│   │
│  ├──────────┤ ├──────────┤ ├──────────┤ ├──────────┤   │
│  │🫵向上管理│ │⚗️需求炼金│ │🥧产品画饼│ │🎨PPT雕花│   │
│  ├──────────┤ ├──────────┤ ├──────────┤ ├──────────┤   │
│  │📊周报生成│ │💬自由对话│ │📋待办列表│ │📁知识库  │   │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘   │
└──────────────────────────────────────────────────────────┘
...
> ⚠️ **注意**：单文件模式 (`--onefile`) 首次启动较慢（需解压），推荐改为 `--onedir` 模式加快启动速度。
